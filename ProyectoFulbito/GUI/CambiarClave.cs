...
namespace GUI
{
    public partial class CambiarClaveForm : Form, ITraducible
    {
        ...
        private void btnCambiarC_Click(object sender, EventArgs e)
        {
            if (!AgregarTxt())
            {
                ...
                if (txtContraseñaC.Text == txtContraseñaN.Text)
                {
                    ...
                    LogBitacora.AgregarEvento("Cambio de contraseña", 3, SessionManager.getInstance.usuario);
                }
                else
                {
        ...
        {
            this.Close();
            LogBitacora.AgregarEvento("Salir de cambiar clave", 1, SessionManager.getInstance.usuario);
        }
